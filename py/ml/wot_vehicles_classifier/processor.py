...
def request_tmm(tank_id, engines, guns, suspensions, radios, turrets, **keys):
    ...
    if os.path.isfile(f'./tmcache/{url_link}.json'):
        file = open(f'./tmcache/{url_link}.json', 'r')
        tmm_json = json.load(open(f'./tmcache/{url_link}.json'))
    else:
    ...
    if not os.path.isfile(f'./tmcache/{url_link}.json'):
        file = open(f'./tmcache/{url_link}.json', 'w')
